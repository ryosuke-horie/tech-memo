...
package.jsonでは`"ts-node": "^10.9.2"`の記載がありますが、これは**バージョン10.9.2以上で互換性があるマイナーバージョン**（例：10.10.0、10.11.0 など）を許容するという意味になります。
特定のバージョンが利用されていることを保証するためにpackage-lock.jsonが利用されます。

```bash
npm ci
```
このコマンドを利用することでpackage-lock.jsonで定義された特定のバージョンをインストールすることができます。
環境差異をなくすために開発時は`npm i`ではなくこちらを利用してください。
